...
builder.Services.AddRazorHxComponents(options => { options.RootComponent = typeof(MainLayout); });
...
app.MapPost("/post", ([FromForm] SampleModel sampleModel) =>
{
    var bla = sampleModel;
    return new RazorHxResult<Get>(new { Model = sampleModel });
}).DisableAntiforgery();
